            ...
            () =>
                pluginExternals(
                    config.includeExternals === false
                        ? {}
                        : (id: string, external: boolean, importer: string) => includeExternals(importer, external, id, config)
                ),
            Priority.externals,
            { format }
        );
    ...
    if (!allowGenericUmd && config.umdInputs.length > 0) {
        ...
        for (const currentInput of config.umdInputs) {
            const isExternal = curry(
                (currentInput: string, id: string, external: boolean, importer: string) =>
...
function includeExternals(_importer: string, external: boolean, id: string, config: CliOptions) {
