...
let generate: <T extends object>(object: T) => string | boolean | RegExp | null | undefined, generateGlobals: () => string;
...
export async function createEjectProvider(preimportMap: Map<string, Promise<never>>) {
    ...
    return [
        {
            provide: (
                plugin: PkgbldRollupPlugin['plugin'],
                priority: PkgbldRollupPlugin['priority'],
                options?: Omit<PkgbldRollupPlugin, 'plugin' | 'priority'>
            ) => {
        ...
        plugins,
    ] as [Provider, PkgbldRollupPlugin[]];
}

export async function ejectConfig(
